"""
deploy.py — Deploy BigQuery Data Agent A2A Proxy to Cloud Run and register with Gemini Enterprise.
"""
import os
import sys
import json
...
LOCATION   = os.getenv("DISCOVERY_LOCATION",  "us")
...
def create_authorization(token, auth_id):
    ...
    default_scopes = f"api://{client_id}/access_as_user openid offline_access"
    ...
    params["redirect_uri"] = ["https://vertexaisearch.cloud.google.com/oauth-redirect"]
...
def manage_authorization(service_name, agent_name="bq-dataagent-a2a"):
...
def main():
    ...
    env_vars = ",".join([
        ...
        f"GCP_PROJECT_ID={os.getenv('GCP_PROJECT_ID', '')}",
        f"GCP_LOCATION={os.getenv('GCP_LOCATION', 'global')}",
        f"DATA_AGENT_ID={os.getenv('DATA_AGENT_ID', '')}",
        f"AGENT_DESCRIPTION={os.getenv('AGENT_DESCRIPTION', '')}",
    ])
